...
__constant  uchar aes_sbox[16][16] = {
...
__constant  uchar aes_invsbox[16][16] = {
...
inline void AddRoundKey(uchar state[][4], __global uint w[])
{
   uchar subkey[4];
...
inline void SubBytes(uchar state[][4])
{
...
inline void ShiftRows(uchar state[][4])
{
   int t;
   // Shift left by 1
   t = state[1][0];
...
inline void MixColumns(uchar state[][4])
{
   uchar col[4];
   // Column 1
   col[0] = state[0][0];
...
__kernel void aes_encrypt(__global uchar16* in,__global uchar16* out,
							__global uint* key, int keysize, int noOfIterations)
{
	int index = get_global_id(0);
	uchar state[4][4];

	// Copy input array (should be 16 bytes long) to a matrix (sequential bytes are ordered
	// by row, not col) called "state" for processing.0
	...
	if (index < noOfIterations){ 
		state[0][0] = in[index].s0;
		state[1][0] = in[index].s4;
		state[2][0] = in[index].s8;
		state[3][0] = in[index].sC;
		state[0][1] = in[index].s1;
		state[1][1] = in[index].s5;
		state[2][1] = in[index].s9;
		state[3][1] = in[index].sD;
		state[0][2] = in[index].s2;
		state[1][2] = in[index].s6;
		state[2][2] = in[index].sA;
		state[3][2] = in[index].sE;
		state[0][3] = in[index].s3;
		state[1][3] = in[index].s7;
		state[2][3] = in[index].sB;
		state[3][3] = in[index].sF;
	   ...
	   // The last round does not perform the MixColumns step.
		...
		ShiftRows(state);
		MixColumns(state); 
		AddRoundKey(state,&key[4]); SubBytes(state); ShiftRows(state); MixColumns(state); 
		AddRoundKey(state,&key[8]); SubBytes(state); ShiftRows(state); MixColumns(state); 
		AddRoundKey(state,&key[12]);SubBytes(state); ShiftRows(state); MixColumns(state); 
		AddRoundKey(state,&key[16]);SubBytes(state); ShiftRows(state); MixColumns(state); 
		AddRoundKey(state,&key[20]);SubBytes(state); ShiftRows(state); MixColumns(state); 
		AddRoundKey(state,&key[24]);SubBytes(state); ShiftRows(state); MixColumns(state); 
		AddRoundKey(state,&key[28]);SubBytes(state); ShiftRows(state); MixColumns(state); 
		AddRoundKey(state,&key[32]);SubBytes(state); ShiftRows(state); MixColumns(state); 
		...
		if (keysize != 128) {
			...
			if (keysize != 192) {
				...
				SubBytes(state); ShiftRows(state); AddRoundKey(state,&key[56]); //last for 256 no mix colums
			}
			else {
				SubBytes(state); ShiftRows(state); AddRoundKey(state,&key[48]); //last for 196 no mix colums
			}
		}
		else {
			SubBytes(state); ShiftRows(state); AddRoundKey(state,&key[40]); //last for 128 no mix colums
		}
	   // Copy the state to the output array*/
	   out[index].s0 = state[0][0];
	   out[index].s4 = state[1][0];
	   out[index].s8 = state[2][0];
	   out[index].sC = state[3][0];
	   out[index].s1 = state[0][1];
	   out[index].s5 = state[1][1];
	   out[index].s9 = state[2][1];
	   out[index].sD = state[3][1];
	   out[index].s2 = state[0][2];
	   out[index].s6 = state[1][2];
	   out[index].sA = state[2][2];
	   out[index].sE = state[3][2];
	   out[index].s3 = state[0][3];
	   out[index].s7 = state[1][3];
	   out[index].sB = state[2][3];
	   out[index].sF = state[3][3];
   } 
}
